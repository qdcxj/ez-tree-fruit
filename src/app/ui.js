...
import { BarkType, FruitType, LeafType, loadPresetWithTextures, preloadTreeTextures } from './textures';
...
export function setupUI(tree, environment, renderer, scene, camera, orbitControls, initialPreset) {
  ...
  const presetSelect = createSelect('Preset',
    ...
    initialPreset,
    (val) => {
      loadPresetWithTextures(tree, val).then(() => refreshAllControls());
    }
  );
  ...
  // ----- Fruits Section -----
  const fruitsSection = createSection('Fruits', 'cube', false);

  const fruitsEnabledToggle = createToggle('Enabled', tree.options.fruits.enabled, (val) => {
    tree.options.fruits.enabled = val;
    onChange();
  });
  fruitsSection.add(fruitsEnabledToggle);
  controls.push({ control: fruitsEnabledToggle, update: () => fruitsEnabledToggle.setValue(tree.options.fruits.enabled) });

  const fruitTypeSelect = createSelect('Type', FruitType, tree.options.fruits.type, (val) => {
    tree.options.fruits.type = val;
    updateFruitTypeControls();
    onChange();
  });
  fruitsSection.add(fruitTypeSelect);
  controls.push({ control: fruitTypeSelect, update: () => fruitTypeSelect.setValue(tree.options.fruits.type) });

  const fruitTintPicker = createColorPicker('Tint', tree.options.fruits.tint, (val) => {
    tree.options.fruits.tint = val;
    onChange();
  });
  fruitsSection.add(fruitTintPicker);
  controls.push({ control: fruitTintPicker, update: () => fruitTintPicker.setValue(tree.options.fruits.tint) });

  const fruitBranchLevelSlider = createSlider('Branch Level', tree.options.fruits.branchLevel, 0, 3, 1, (val) => {
    tree.options.fruits.branchLevel = val;
    onChange();
  });
  fruitsSection.add(fruitBranchLevelSlider);
  controls.push({ control: fruitBranchLevelSlider, update: () => fruitBranchLevelSlider.setValue(tree.options.fruits.branchLevel) });

  const fruitCountSlider = createSlider('Count', tree.options.fruits.count, 0, 100, 1, (val) => {
    tree.options.fruits.count = val;
    onChange();
  });
  fruitsSection.add(fruitCountSlider);
  controls.push({ control: fruitCountSlider, update: () => fruitCountSlider.setValue(tree.options.fruits.count) });

  const fruitStartSlider = createSlider('Start', tree.options.fruits.start, 0, 1, 0.01, (val) => {
    tree.options.fruits.start = val;
    onChange();
  });
  fruitsSection.add(fruitStartSlider);
  controls.push({ control: fruitStartSlider, update: () => fruitStartSlider.setValue(tree.options.fruits.start) });

  const fruitSizeSlider = createSlider('Size', tree.options.fruits.size, 0.05, 2, 0.01, (val) => {
    tree.options.fruits.size = val;
    onChange();
  });
  fruitsSection.add(fruitSizeSlider);
  controls.push({ control: fruitSizeSlider, update: () => fruitSizeSlider.setValue(tree.options.fruits.size) });

  const fruitVarianceSlider = createSlider('Size Variance', tree.options.fruits.sizeVariance, 0, 1, 0.01, (val) => {
    tree.options.fruits.sizeVariance = val;
    onChange();
  });
  fruitsSection.add(fruitVarianceSlider);
  controls.push({ control: fruitVarianceSlider, update: () => fruitVarianceSlider.setValue(tree.options.fruits.sizeVariance) });

  const fruitSegmentsSlider = createSlider('Segments', tree.options.fruits.segments, 4, 32, 1, (val) => {
    tree.options.fruits.segments = val;
    onChange();
  });
  fruitsSection.add(fruitSegmentsSlider);
  controls.push({ control: fruitSegmentsSlider, update: () => fruitSegmentsSlider.setValue(tree.options.fruits.segments) });

  const appleFruitControls = [];

  const fruitShininessSlider = createSlider('Shininess', tree.options.fruits.shininess, 0, 100, 1, (val) => {
    tree.options.fruits.shininess = val;
    onChange();
  });
  fruitsSection.add(fruitShininessSlider);
  appleFruitControls.push(fruitShininessSlider.element);
  controls.push({ control: fruitShininessSlider, update: () => fruitShininessSlider.setValue(tree.options.fruits.shininess) });

  const fruitCapScaleSlider = createSlider('Cap Scale', tree.options.fruits.capScale, 0.7, 1.3, 0.01, (val) => {
    tree.options.fruits.capScale = val;
    onChange();
  });
  fruitsSection.add(fruitCapScaleSlider);
  appleFruitControls.push(fruitCapScaleSlider.element);
  controls.push({ control: fruitCapScaleSlider, update: () => fruitCapScaleSlider.setValue(tree.options.fruits.capScale) });

  const glassFruitControls = [];

  const fruitTransmissionSlider = createSlider('Transmission', tree.options.fruits.transmission, 0, 1, 0.01, (val) => {
    tree.options.fruits.transmission = val;
    onChange();
  });
  fruitsSection.add(fruitTransmissionSlider);
  glassFruitControls.push(fruitTransmissionSlider.element);
  controls.push({ control: fruitTransmissionSlider, update: () => fruitTransmissionSlider.setValue(tree.options.fruits.transmission) });

  const fruitRoughnessSlider = createSlider('Roughness', tree.options.fruits.roughness, 0, 1, 0.01, (val) => {
    tree.options.fruits.roughness = val;
    onChange();
  });
  fruitsSection.add(fruitRoughnessSlider);
  glassFruitControls.push(fruitRoughnessSlider.element);
  controls.push({ control: fruitRoughnessSlider, update: () => fruitRoughnessSlider.setValue(tree.options.fruits.roughness) });

  const fruitIorSlider = createSlider('IOR', tree.options.fruits.ior, 1, 2.5, 0.01, (val) => {
    tree.options.fruits.ior = val;
    onChange();
  });
  fruitsSection.add(fruitIorSlider);
  glassFruitControls.push(fruitIorSlider.element);
  controls.push({ control: fruitIorSlider, update: () => fruitIorSlider.setValue(tree.options.fruits.ior) });

  const fruitThicknessSlider = createSlider('Thickness', tree.options.fruits.thickness, 0.05, 2, 0.01, (val) => {
    tree.options.fruits.thickness = val;
    onChange();
  });
  fruitsSection.add(fruitThicknessSlider);
  glassFruitControls.push(fruitThicknessSlider.element);
  controls.push({ control: fruitThicknessSlider, update: () => fruitThicknessSlider.setValue(tree.options.fruits.thickness) });

  const fruitClearcoatSlider = createSlider('Clearcoat', tree.options.fruits.clearcoat, 0, 1, 0.01, (val) => {
    tree.options.fruits.clearcoat = val;
    onChange();
  });
  fruitsSection.add(fruitClearcoatSlider);
  glassFruitControls.push(fruitClearcoatSlider.element);
  controls.push({ control: fruitClearcoatSlider, update: () => fruitClearcoatSlider.setValue(tree.options.fruits.clearcoat) });
  ...
  parametersTab.appendChild(fruitsSection.element);
