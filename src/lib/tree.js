...
import { createAppleBodyMaterial, createAppleStemMaterial, createGlassBallMaterial, applyAppleSphereUVs, applyStemUVs } from './fruitMaterial';
...
export class Tree extends THREE.Group {
  ...
  constructor(options = new TreeOptions()) {
    ...
    this.fruitsMesh = null;
    this.fruitStemsMesh = null;
  ...
  generateBranch(branch) {
    ...
    if (
      this.options.fruits.enabled &&
      branch.level === this.getFruitBranchLevel()
    ) {
      this.generateFruits(sections);
    }
  }

  /**
   * Branch level used for fruit placement. Defaults to the tree's leaf level.
   */
  getFruitBranchLevel() {
    const levels = this.options.branch.levels;
    const configured = this.options.fruits.branchLevel;
    return configured <= levels ? configured : levels;
  }
  ...
  */
  ...
  /**
   * Spawn fruit along a branch using the same stratified sampling as leaves.
   * @param {{
   *  origin: THREE.Vector3,
   *  orientation: THREE.Euler,
   *  radius: number
  ...
  generateFruits(sections) {
    ...
    for (let i = 0; i < count; i++) {
      let fruitStart = startMin + (i + this.rng.random()) * heightStep;

      const sectionIndex = Math.floor(fruitStart * (sections.length - 1));
      ...
      const alpha =
        (fruitStart - sectionIndex / (sections.length - 1)) /
        (1 / (sections.length - 1));

      const fruitOrigin = new THREE.Vector3().lerpVectors(
        sectionA.origin,
        sectionB.origin,
        alpha,
      );
      ...
      const fruitOrientation = new THREE.Euler().setFromQuaternion(
        q3.multiply(q2.multiply(q1)),
      );
  ...
  /**
   * Records a single fruit instance for instanced rendering.
   * @param {THREE.Vector3} origin
   * @param {THREE.Euler} orientation
   * @param {number} branchRadius
   */
  generateFruit(origin, orientation, branchRadius) {
    const size =
      this.options.fruits.size *
      (1 +
        this.rng.random(
          this.options.fruits.sizeVariance,
          -this.options.fruits.sizeVariance,
        ));

    const hangDistance = branchRadius + size * 1.8;
    const position = origin
      .clone()
      .add(new THREE.Vector3(0, -hangDistance, 0).applyEuler(orientation));

    this.fruits.instances.push({
      position,
      rotation: new THREE.Euler(
        this.rng.random() * Math.PI,
        this.rng.random() * Math.PI * 2,
        this.rng.random() * Math.PI,
      ),
      scale: size,
    });
  ...
  */
  ...
  /**
   * Builds instanced sphere geometry for fruit.
   */
  createFruitsGeometry() {
    if (this.fruitsMesh) {
      this.remove(this.fruitsMesh);
      this.fruitsMesh.geometry.dispose();
      this.fruitsMesh.material.dispose();
      this.fruitsMesh = null;
    }
    if (this.fruitStemsMesh) {
      this.remove(this.fruitStemsMesh);
      this.fruitStemsMesh.geometry.dispose();
      this.fruitStemsMesh.material.dispose();
      this.fruitStemsMesh = null;
    }

    const instances = this.fruits.instances;
    if (!this.options.fruits.enabled || instances.length === 0) {
      return;
    }

    const fruits = this.options.fruits;
    const isGlassBall = fruits.type === 'glassBall';
    const colorMap = fruits.maps?.color ?? fruits.map ?? null;
    const materialOpts = {
      tint: fruits.tint,
      shininess: fruits.shininess,
      capScale: fruits.capScale ?? 1.0,
      transmission: fruits.transmission ?? 1.0,
      roughness: fruits.roughness ?? 0.05,
      ior: fruits.ior ?? 1.5,
      thickness: fruits.thickness ?? 0.35,
      clearcoat: fruits.clearcoat ?? 1.0,
      clearcoatRoughness: fruits.clearcoatRoughness ?? 0.03,
    };

    const segments = fruits.segments;
    ...
    if (!isGlassBall) {
      ...
      applyStemUVs(stemGeometry);
      const stemMaterial = createAppleStemMaterial(colorMap, materialOpts);
      stemMesh = new THREE.InstancedMesh(stemGeometry, stemMaterial, instances.length);
      stemMesh.name = 'fruit-stems';
      stemMesh.castShadow = true;
      stemMesh.receiveShadow = true;
      stemMesh.frustumCulled = false;
      stemMesh.renderOrder = 3;
    }
    ...
    instances.forEach((instance, index) => {
      ...
      if (stemMesh) {
        ...
        stemMesh.setMatrixAt(index, stemDummy.matrix);
      }

      const half = instance.scale;
      bounds.expandByPoint(
        new THREE.Vector3(
          instance.position.x - half,
          instance.position.y - half,
          instance.position.z - half,
        ),
      );
      bounds.expandByPoint(
        new THREE.Vector3(
    ...
    this.fruitsMesh = mesh;
    this.fruitStemsMesh = stemMesh;
    this.add(this.fruitsMesh);
    if (this.fruitStemsMesh) {
      this.add(this.fruitStemsMesh);
    }
  }
  ...
  get vertexCount() {
    let count = (this.branches.verts.length + this.leaves.verts.length) / 3;
    if (this.fruitsMesh) {
      count += this.fruitsMesh.count * this.fruitsMesh.geometry.attributes.position.count;
    }
    return count;
  }

  get triangleCount() {
    let count = (this.branches.indices.length + this.leaves.indices.length) / 3;
    if (this.fruitsMesh) {
      const indexCount = this.fruitsMesh.geometry.index
        ? this.fruitsMesh.geometry.index.count
        : this.fruitsMesh.geometry.attributes.position.count;
      count += (this.fruitsMesh.count * indexCount) / 3;
    }
    return count;
  }
}
