...
export default class TreeOptions {
  constructor() {
    ...
    // Fruit parameters (guoshi)
    this.fruits = {
      // Whether to generate fruit on the tree
      enabled: true,

      // Informational identifier (e.g. 'apple'). The host app resolves this
      // to `map` below.
      type: 'apple',
      ...
      // PBR maps supplied by the caller.
      maps: {
        color: null,
        normal: null,
        gloss: null,
      ...
      // Branch level where fruit is attached (typically the twig level)
      branchLevel: 3,

      // Number of fruit instances
      count: 8,

      // Where fruit starts along the branch length (0 to 1)
      start: 0.3,

      // Base radius of each fruit sphere
      size: 0.6,

      // Random size variation per instance
      sizeVariance: 0.2,
      ...
      // Cap projection scale for scan atlas (FruitAppleRedWhole001)
      capScale: 1.0,

      // Glass ball (type: 'glassBall') — MeshPhysicalMaterial
      transmission: 1.0,
      roughness: 0.05,
      ior: 1.5,
      thickness: 0.35,
